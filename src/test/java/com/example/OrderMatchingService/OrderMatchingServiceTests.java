...
class OrderMatchingServiceTests {
	...
	void givenPartialMatch_thenUnmatchedPartRemainsInBook() {
		...
		assertTrue(orderBook.getReservedOrder(sellOrder.getOrderID()).isEmpty());
		assertTrue(orderBook.getReservedOrder(buyOrder.getOrderID()).isPresent());
	...
	void givenMultipleSellOrdersAtSamePrice_whenBuyOrder_thenFifoPriorityUsed() {
		...
		OrderMatchedEvent event = orderMatcher.match(buyOrder).get(0);
		assertEquals(sell1.getUserId(), event.getSellUserId());
	}
